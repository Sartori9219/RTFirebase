      ...
      if (Array.isArray(questions[currentQuestion].hints)) {
        if (questions[currentQuestion].hints.length > 0) {
          for (let i = 0; i < questions[currentQuestion].hints.length; i++) {
            const hint = questions[currentQuestion].hints[i];
            const isBought = hint.isBought || false;
            if (!isBought) {
              if (i === 0) {
                button = (
                  <Pressable
          ...
          function updateData() {
            const newQuestions = questions.filter((question) => {
              if (question === questions[currentQuestion]) {
                ...
                updatedHints[i] = {...updatedHints[i], isBought: true};
                return {...question, hints: updatedHints};
              }
              return question;
            });
